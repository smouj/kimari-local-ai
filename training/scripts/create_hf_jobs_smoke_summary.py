...
def create_summary(job_id: str, status: str, output_path: str = "") -> dict:
...
def main() -> None:
    ...
    parser.add_argument(
        "--status", required=True, choices=["completed", "failed", "running", "timed_out"], help="Job status"
    )
    parser.add_argument("--output", default="", help="Output file path")
    parser.add_argument("--json", action="store_true", help="JSON output")
    args = parser.parse_args()

    summary = create_summary(args.job_id, args.status, args.output)
