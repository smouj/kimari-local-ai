.PHONY: help doctor info start-1060 start-1080 start-turbo start-test start-docker status stop chat smoke bench bench-1080 bench-1060 clean install install-dev dry-run-test dry-run logs logs-follow doctor-json status-json validate-config validate-schema webui-up webui-down webui-logs test lint ci-local pull-list pull-test models profiles config-validate config-show config-path migrate format-check build-rocm
...
bench: ## Run benchmarks with default profile (test)
	python -m kimari.cli.main bench --profile test

bench-1080: ## Run benchmarks with GTX 1080 profile
	python -m kimari.cli.main bench --profile gtx1080

bench-1060: ## Run benchmarks with GTX 1060 profile
	python -m kimari.cli.main bench --profile gtx1060

dry-run-test: ## Preview server start without running (test profile)
	python -m kimari.cli.main start --profile test --dry-run

dry-run: ## Preview server start without running (default profile)
	python -m kimari.cli.main start --dry-run
...
ci-local: ## Run local CI: validate-config, py_compile, lint, format, and pytest
	...
	@echo "[3/5] lint (ruff check)"
	@ruff check kimari/ tests/
	@echo "[4/5] format check (ruff format)"
	@ruff format --check kimari/ tests/
	@echo "[5/5] pytest"
	python -m pytest tests/ -q
