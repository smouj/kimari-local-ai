#!/usr/bin/env python3
"""CLI for baseline eval planning.

Produces an evaluation plan for the base model (before SFT):
- model_label
- prompts_path
...
def build_baseline_eval_plan(
    model_label: str,
    prompts_path: Path,
    dry_run: bool,
) -> dict:
    """Build the baseline eval plan."""
    # Load prompts
    ...
    recommended_output = PROJECT_ROOT / "eval" / "results" / f"baseline-{safe_label}-private.json"
    ...
    steps = [
        {
            "step": 1,
            "action": "start_server",
            "description": "Start llama-server with the base model",
            "command": "kimari start --profile <profile>  # or manual llama-server invocation",
            "dry_run_skip": True,
        },
        {
            "step": 2,
        ...
        },
        {
            "step": 3,
            "action": "run_eval",
            "description": "Run KimariFit eval against the base model",
        ...
        },
        {
            ...
            f"--input {recommended_output} --output eval/results/baseline-{safe_label}-summary.json",
            "dry_run_skip": True,
        },
        {
            "step": 5,
        ...
        },
        {
            ...
            "command": f"git add eval/results/baseline-{safe_label}-summary.json",
            "dry_run_skip": True,
        },
    ]
...
def main() -> None:
    """CLI entry point for baseline eval planning."""
    parser = argparse.ArgumentParser(
        description="Baseline eval planning for Kimari models. No real training. No model downloads. No HF uploads.",
    )
    parser.add_argument(
        "--model-label",
        default="smollm3-base",
        help="Label for the base model being evaluated (default: smollm3-base)",
    )
    parser.add_argument(
        "--prompts",
        type=Path,
    ...
    if args.json_output:
        ...
        print(f"  Baseline Eval Plan [{mode}]")
        print("=" * 60)
        print()
        ...
        if plan["dry_run"]:
            ...
            print("  Ensure llama-server is running before executing.")
        print("=" * 60)
