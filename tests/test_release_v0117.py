"""Release validation tests for Kimari Local AI v0.1.17-alpha.
...
- Version consistency (pyproject.toml == __init__.py == "0.1.17-alpha")
"""
...
class TestVersion:
    def test_version_is_0117(self):
        ...
        assert __version__ == "0.1.17-alpha"

    def test_pyproject_version_matches(self):
        pyproject = PROJECT_ROOT / "pyproject.toml"
        text = pyproject.read_text()
        assert 'version = "0.1.17-alpha"' in text

    def test_cli_info_version(self):
        result = _run_kimari("info", "--json")
        assert result.returncode == 0
        data = json.loads(result.stdout)
        assert data["kimari_version"] == "0.1.17-alpha"
