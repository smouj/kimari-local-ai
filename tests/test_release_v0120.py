"""Release validation tests for Kimari Local AI v0.1.20-alpha.

Tests cover:
- Version consistency (0.1.20-alpha)
...
class TestVersion:
    def test_version_in_init(self):
        ...
        assert __version__ == "0.1.20-alpha"

    def test_version_in_pyproject(self):
        text = (PROJECT_ROOT / "pyproject.toml").read_text()
        assert 'version = "0.1.20-alpha"' in text
...
class TestModelCardFixes:
    ...
    def test_model_card_version_history_0120(self):
        content = (PROJECT_ROOT / "MODEL_CARD.md").read_text()
        assert "0.1.20-alpha" in content, "MODEL_CARD version history should include 0.1.20-alpha"
