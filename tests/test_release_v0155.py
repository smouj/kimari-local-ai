...
def test_public_version_consistency_script_passes():
    ...
    assert payload["version"] == "0.1.58-alpha"
...
def test_public_surfaces_current_and_safe():
    ...
    assert (
        "version-0.1.58--alpha" in (PROJECT_ROOT / "README.md").read_text()
        or "0.1.58" in (PROJECT_ROOT / "README.md").read_text()
    )
...
def test_release_check_mentions_public_v0155_checks():
    ...
    assert "v0.1.58" in text or "v0.1.57" in text
