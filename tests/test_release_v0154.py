...
def test_version_bumped_to_v0154():
    assert (
        ...
        or 'version = "0.1.55-alpha"' in (PROJECT_ROOT / "pyproject.toml").read_text()
    )
    assert (
        '__version__ = "0.1.54-alpha"' in (PROJECT_ROOT / "kimari" / "__init__.py").read_text()
        or '__version__ = "0.1.55-alpha"' in (PROJECT_ROOT / "kimari" / "__init__.py").read_text()
    )
...
def test_public_docs_show_current_version_and_not_released():
    ...
    assert "v0.1.54-alpha" in readme or "v0.1.55-alpha" in readme
    assert "version-0.1.54--alpha" in readme or "version-0.1.55--alpha" in readme
    assert "version-v0.1.53--alpha" not in readme
    assert "v0.1.54-alpha" in docs_index or "v0.1.55-alpha" in docs_index
    assert "Kimari Local AI v0.1.28-alpha" not in docs_index
    assert "New in v0.1.28-alpha" not in docs_index
    assert "v0.1.54-alpha" in org_card or "v0.1.55-alpha" in org_card
    assert "v0.1.54-alpha" in deploy or "v0.1.55-alpha" in deploy
    assert "v0.1.54-alpha" in space or "v0.1.55-alpha" in space
