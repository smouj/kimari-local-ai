...
def test_version_bumped_to_v0154():
    assert 'version = "0.1.58-alpha"' in (PROJECT_ROOT / "pyproject.toml").read_text()
    assert '__version__ = "0.1.58-alpha"' in (PROJECT_ROOT / "kimari" / "__init__.py").read_text()
...
def test_public_docs_show_current_version_and_not_released():
    ...
    assert "v0.1.58-alpha" in readme or "0.1.58-alpha" in readme
    assert "version-0.1.58--alpha" in readme or "0.1.58" in readme
    assert "version-v0.1.53--alpha" not in readme
    assert "v0.1.58-alpha" in docs_index
    assert "Kimari Local AI v0.1.28-alpha" not in docs_index
    assert "New in v0.1.28-alpha" not in docs_index
    assert "v0.1.58-alpha" in org_card or "0.1.58" in org_card
    assert "v0.1.58" in deploy or "0.1.58-alpha" in deploy
    assert "v0.1.58" in space or "0.1.58-alpha" in space
